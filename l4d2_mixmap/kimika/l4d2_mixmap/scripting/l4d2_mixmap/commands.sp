...
void CreateMixmapMenu(int client)
{
	...
	FormatEx(sBuffer, sizeof(sBuffer), "%T", "MenuItem_AutoPreset", client);
	menu.AddItem("", sBuffer);
	...
	switch (action)
	{
		case MenuAction_Select:
		{
			switch (selection)
			{
				...
				case 1: LoadPreset_CreateFileMenu(client, true);
				case 2: ManullySelectMap_ChooseMode(client);
				case 3: LoadPreset_CreateFileMenu(client, false);
			}
		}
...
void ManullySelectMap_ChooseMode(int client)
{
	char sBuffer[128];
	Menu menu = new Menu(MenuHandler_ChooseManualMode);

	FormatEx(sBuffer, sizeof(sBuffer), "%T", "MenuTitle_SelectManualMode", client);
	menu.SetTitle(sBuffer);

	FormatEx(sBuffer, sizeof(sBuffer), "%T", "MenuItem_ManualCampaignRandomChapter", client);
	menu.AddItem("", sBuffer);

	FormatEx(sBuffer, sizeof(sBuffer), "%T", "MenuItem_ManualCampaignAndChapter", client);
	menu.AddItem("", sBuffer);
...
void MenuHandler_ChooseManualMode(Menu menu, MenuAction action, int client, int selection)
{
	switch (action)
	{
		case MenuAction_Select:
		{
			switch (selection)
			{
				case 0: ManullySelectMap_ChooseMapSetType(client, true);
				case 1: ManullySelectMap_ChooseMapSetType(client, false);
			}
		}
...
void ManullySelectMap_ChooseMapSetType(int client, bool bCampaignOnly = false)
{
	g_bManualCampaignOnly = bCampaignOnly;
	char sBuffer[128];
...
void LoadPreset_CreateFileMenu(int client, bool bRandom)
{
...
void MenuHandler_LoadPreset(Menu menu, MenuAction action, int param1, int param2)
{
	switch (action)
	{
		case MenuAction_Select:
		{
			char sItem[256], sFile[128];
			menu.GetItem(param2, sItem, sizeof(sItem));

			int iSeparator = FindCharInString(sItem, '|');
			if (iSeparator <= 0 || iSeparator >= strlen(sItem) - 1)
				return;

			bool bRandom = sItem[0] == '1';
			strcopy(sFile, sizeof(sFile), sItem[iSeparator + 1]);
			LoadPreset(sFile, param1, bRandom);
		}
