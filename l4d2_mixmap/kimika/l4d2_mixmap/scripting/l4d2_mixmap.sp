...
#define PLUGIN_VERSION "re3.5.0"

// Keep the existing logging call sites on the built-in SourceMod logger path.
#define MIXMAP_LOG_LEVEL_INFO 1

methodmap MixmapLogger
{
	public MixmapLogger()
	{
		return view_as<MixmapLogger>(1);
	}

	property int IgnoreLevel
	{
		public get()
		{
			return MIXMAP_LOG_LEVEL_INFO;
		}
	}
	...
	public void warning(const char[] format, any ...)
	{
		char message[1024];
		VFormat(message, sizeof(message), format, 3);
		LogError("[Mixmap] [WARNING] %s", message);
	}
...
MixmapLogger g_hLogger;	 // Uses SourceMod's built-in logging functions.

bool	   g_bManullyChoosingMap = false;
bool	   g_bMapsetInitialized	 = false;
bool	   g_bManualCampaignOnly = false;
int		   g_iMapsPlayed		 = 0;
