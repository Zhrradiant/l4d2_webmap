...
bool GetMapRoleForMode(const char[] sMap, const char[] sMode, MapRole &role, int &survivorSet, char[] sMission, int missionSize)
{
	role = MapRole_Invalid;
	survivorSet = 2;
	sMission[0] = '\0';
	...
	for (SourceKeyValues kvMission = kvMissions.GetFirstTrueSubKey(); !kvMission.IsNull(); kvMission = kvMission.GetNextTrueSubKey())
	{
		...
		kvMission.GetName(sMissionName, sizeof(sMissionName));
		if (IsFakeMission(sMissionName))
			continue;

		char sKey[128];
		FormatEx(sKey, sizeof(sKey), "modes/%s", sMode);
		SourceKeyValues kvMode = kvMission.FindKey(sKey);
		if (!kvMode || kvMode.IsNull())
			continue;

		int mapCount = 0;
		int mapIndex = -1;
		for (SourceKeyValues kvMap = kvMode.GetFirstTrueSubKey(); !kvMap.IsNull(); kvMap = kvMap.GetNextTrueSubKey())
		{
			char sCandidate[64];
			kvMap.GetString("Map", sCandidate, sizeof(sCandidate));
			if (StrEqual(sCandidate, sMap, false))
				mapIndex = mapCount;
			mapCount++;
		}

		if (mapIndex == -1 || mapCount <= 0)
			continue;

		survivorSet = kvMission.GetInt("survivor_set", 2);
		strcopy(sMission, missionSize, sMissionName);
		if (mapCount == 1)
			role = MapRole_Single;
		else if (mapIndex == 0)
			role = MapRole_First;
...
bool GetCurrentMapRole(const char[] sMap, MapRole &role, int &survivorSet, char[] sMission, int missionSize)
{
	char sMode[64];
	FindConVar("mp_gamemode").GetString(sMode, sizeof(sMode));
	GetBasedMode(sMode, sizeof(sMode));
	return GetMapRoleForMode(sMap, sMode, role, survivorSet, sMission, missionSize);
}
...
/*
	...
	{
		g_hLogger.debug("### GetSafeAreaOrigin: Failed to find checkpoint entity. checkPoint: %d. Abort action.", checkPoint);
	}
*/
...
void GetSafeAreaOriginEx(float vec[3])
{
	...
	/*
		...
		g_hLogger.debug("### GetSafeAreaOrigin: pCheckPoint: %d.", pCheckPoint);
		if (pCheckPoint != Address_Null)
		{
			Address pLargest = SDKCall(g_hSDKCall_GetLargestArea, pCheckPoint);
			g_hLogger.debug("### GetSafeAreaOrigin: pLargest: %d.", pLargest);
...
// Resolve mutation/community modes without l4d2_source_keyvalues SDKCalls.
stock void GetBasedMode(char[] sMode, int size)
{
	int modeType = L4D_GetGameModeType();
	switch (modeType)
	{
		case GAMEMODE_COOP:
			strcopy(sMode, size, "coop");
		case GAMEMODE_VERSUS:
			strcopy(sMode, size, "versus");
		case GAMEMODE_SCAVENGE:
			strcopy(sMode, size, "scavenge");
		default:
		{
			// Survival is not a supported Mixmap base mode. Preserve the original
			// mp_gamemode value so the existing no-mode/no-map handling can report it.
